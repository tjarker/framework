package object framework {
  ...
  export simulation.stepClockDomain
  ...
  export framework.types.Extensions.*
  ...
  inline def param[T: ClassTag](using Hierarchy): T = {
    Config
      ...
      .getOrElse(
        throw new Exception(
          s"Object ${summon[Hierarchy].name} expects parameter ${macros.Naming.enclosingTermName}"
        )
      )
      .asInstanceOf[T]
  }

  inline def create[T](name: String, c: Hierarchy ?=> T, conf: Map[Any, Any])(
      using Hierarchy
  ): T = {
    val newHierarchy =
      Factory.setupEnvironment(summon[Hierarchy], name, conf, Map.empty)
  ...
  inline def create[T](c: Hierarchy ?=> T, conf: Map[Any, Any])(using
      Hierarchy
  ): T = {

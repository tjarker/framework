...
class Logger(enable: Boolean) {
  ...
  inline def info(provider: String, msg: String): Unit = if (enable) {
    ...
    inline def error(provider: String, msg: String): Unit = if (enable) {
      val lines = msg.split("\n")
      Console.err.println(s"[${RED}error${RESET}] [${YELLOW}$provider${RESET}] ${lines.head} (${BLUE}${FileContext()}${RESET})")
      lines.tail.foreach { line =>
        Console.err.println(s"[${RED}error${RESET}] [${YELLOW}$provider${RESET}] ${line}")
      }
    }
